...
const addMedicalHistory = async(req, res, next)=>{
    const{patientId, appointmentDate, department, doctor, requiredReports, comments} = req.body;

    let medicalHistory;

    try{
        medicalHistory = new MedicalHistory({patientId, appointmentDate, department, doctor, requiredReports, comments});
...
const updatemedicalHistory = async(req, res, next) =>{
    ...
    try{
        medicalHistory = await MedicalHistory.findByIdAndUpdate(id, {patientId: patientId, appointmentDate: appointmentDate, department: department, doctor: doctor, requiredReports: requiredReports, comments: comments});
